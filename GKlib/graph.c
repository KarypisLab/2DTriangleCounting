                ...
                int numbering, int isfewgts, int isfvwgts, int isfvsizes)
{
  ...
  ssize_t *xadj;
  int32_t ival, *iinds=NULL, *jinds=NULL, *ivals=NULL, *iadjwgt;
  //int32_t *adjncy; 
  adj_t *adjncy;
  ...
  switch (format) {
    case GK_GRAPH_FMT_METIS:
      ...
      graph->adjncy = malloc(nedges * sizeof(adj_t));
      if (readvals) {
        if (isfewgts)
    ...
    case GK_GRAPH_FMT_IJV:
      ...
      adjncy = graph->adjncy = malloc(nedges * sizeof(adj_t));
      if (hasvals) {
        if (isfewgts)
...
void gk_graph_Write(gk_graph_t *graph, char *filename, int format, int numbering)
{
  ...
  switch (format) {
    ...
    case GK_GRAPH_FMT_METIS:
      ...
      for (i=0; i<graph->nvtxs; i++) {
        ...
        for (j=graph->xadj[i]; j<graph->xadj[i+1]; j++) {
          fprintf(fpout, " %"PRId64"", graph->adjncy[j]+1);
          if (hasewgts) {
            if (graph->iadjwgt)
    ...
    case GK_GRAPH_FMT_IJV:
      for (i=0; i<graph->nvtxs; i++) {
        for (j=graph->xadj[i]; j<graph->xadj[i+1]; j++) {
          fprintf(fpout, "%zd %zd ", i+numbering, (ssize_t)graph->adjncy[j]+numbering);
          if (hasewgts) {
            if (graph->iadjwgt)
              fprintf(fpout, " %d\n", graph->iadjwgt[j]);
            else 
...
gk_graph_t *gk_graph_Dup(gk_graph_t *graph)
{
  ...
  #endif
  if (graph->adjncy)
    ngraph->adjncy = gk_i64copy(graph->xadj[graph->nvtxs], graph->adjncy, 
                            malloc(graph->xadj[graph->nvtxs] * sizeof(adj_t)));
  if (graph->iadjwgt)
...
gk_graph_t *gk_graph_ExtractSubgraph(gk_graph_t *graph, int vstart, int nvtxs)
{
  ...
  #endif
  if (graph->adjncy)
    ngraph->adjncy = gk_i64copy(graph->xadj[vstart+nvtxs]-graph->xadj[vstart], 
                            graph->adjncy+graph->xadj[vstart], 
                            malloc(graph->xadj[vstart+nvtxs]-graph->xadj[vstart]*sizeof(adj_t)));
...
gk_graph_t *gk_graph_Reorder(gk_graph_t *graph, int32_t *perm, int32_t *iperm)
{
  ...
  #endif
  if (graph->adjncy)
    ngraph->adjncy = malloc(graph->xadj[nvtxs]*sizeof(adj_t));
          ...
          int32_t **r_perm, int32_t **r_iperm)
{
  ssize_t j, jj, *xadj;
  int i, k, u, nvtxs;
  int32_t *perm, *degrees, *minIDs, *open;
  adj_t *adjncy;
  gk_i32pq_t *queue;
          ...
          int32_t **r_perm, int32_t **r_iperm)
{
  ...
  int32_t *perm, *degrees, *sod, *level, *ot, *pos;
  adj_t *adjncy;
  int64_t *wdegrees;
  gk_i32pq_t *queue;
